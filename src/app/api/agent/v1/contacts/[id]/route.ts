...
export const PATCH = withAgentRoute<{ params: Promise<{ id: string }> }>(async (
  ...
  if (body.addTags !== undefined) {
    if (!Array.isArray(body.addTags) || body.addTags.some((t) => typeof t !== "string")) {
      return agentError("VALIDATION_FAILED", "addTags must be an array of strings.", 400);
    }
  }
  if (body.removeTags !== undefined) {
    if (!Array.isArray(body.removeTags) || body.removeTags.some((t) => typeof t !== "string")) {
      return agentError("VALIDATION_FAILED", "removeTags must be an array of strings.", 400);
    }
  }
