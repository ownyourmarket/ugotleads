...
beforeEach(() => {
  ...
  fakeDb.doc("agencyServiceKeys/key1").set({
    ...
    scopes: ["sequences:enroll", "reports:read"],
    status: "active",
  });
  KEY = gen.key;
...
describe("agent sequence enroll/unenroll/status", () => {
  it("refuses enrollment without a matching confirm (batch-approval gate)", async () => {
    ...
    const badCount = await ENROLL(
      post({
        contactIds: ["c1", "c2"],
        confirm: { expectedCount: 5, summary: "x" },
      }),
      ctx
    );
  ...
  it("enrolls by ids with confirm; re-run is a clean catch-up", async () => {
    const res = await ENROLL(
      post({
        contactIds: ["c1", "c2"],
        confirm: { expectedCount: 2, summary: "Box1 batch" },
      }),
      ctx
    );
    ...
    expect((await rerun.json()).data).toMatchObject({
      enrolled: 0,
      alreadyEnrolled: 2,
    });
  });
  ...
  it("enrolls by tag and reports skips for unknown ids", async () => {
    ...
    const withGhost = await ENROLL(
      post({
        contactIds: ["ghost"],
        confirm: { expectedCount: 1, summary: "g" },
      }),
      ctx
    );
    expect((await withGhost.json()).data.skipped).toEqual([
      { contactId: "ghost", reason: "not_found" },
    ]);
  });

  it("unenroll stops running executions; status rolls up", async () => {
    await ENROLL(
      post({ contactIds: ["c1"], confirm: { expectedCount: 1, summary: "s" } }),
      ctx
    );
    ...
    ).toMatchObject({ status: "stopped", stoppedReason: "manual" });
  ...
  it("enforces the daily enrollment cap in units", async () => {
    ...
    const res = await ENROLL(
      post({
        contactIds: ["c1", "c2"],
        confirm: { expectedCount: 2, summary: "s" },
      }),
      ctx
    );
  ...
  it("replays a cached enroll even after the cap is later reached (replay never re-consumes quota)", async () => {
    const first = await ENROLL(
      post(
        { contactIds: ["c1"], confirm: { expectedCount: 1, summary: "s" } },
        "e1"
      ),
      ctx
    );
    ...
    const execsBefore = (await fakeDb.collection("automation_executions").get())
      .size;
    ...
    const execsAfter = (await fakeDb.collection("automation_executions").get())
      .size;
  ...
  it("does not cache a fresh enroll blocked by the cap", async () => {
    ...
    const beforeCount = (await fakeDb.collection("agentIdempotency").get())
      .size;

    const res = await ENROLL(
      post(
        { contactIds: ["c2"], confirm: { expectedCount: 1, summary: "s" } },
        "e2"
      ),
      ctx
    );
    ...
    expect(
      (await fakeDb.doc("automation_executions/seq1_c2").get()).exists
    ).toBe(false);
  });
});
