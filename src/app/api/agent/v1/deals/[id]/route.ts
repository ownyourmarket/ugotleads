...
export const PATCH = withAgentRoute<{ params: Promise<{ id: string }> }>(
  async (request, ctx) => {
    ...
    if (
      ...
      !PIPELINE_STAGES.some((s) => s.id === body.stageId)
    ) {
      return agentError("VALIDATION_FAILED", "Unknown stageId.", 400);
    }
    if (
      body.priority !== undefined &&
      !DEAL_PRIORITIES.some((p) => p.id === body.priority)
    ) {
    ...
    const stageChanged =
      body.stageId !== undefined && body.stageId !== deal.stageId;
    if (stageChanged) {
    ...
    if (stageChanged) {
      await db
        .collection(`contacts/${deal.contactId as string}/activities`)
        .add({
          type: "pipeline_moved",
